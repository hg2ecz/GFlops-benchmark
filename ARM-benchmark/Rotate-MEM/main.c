...
static void *comp_thread(void *arg) {
    ...
    for (int i=0; i<ITERNUM_MILL; i++) rotate(base, phase);
    printf("Thread#%d re: %f %f %f %f\n", tinfo->thread_num, base[0].re[0], base[0].re[1], base[0].re[2], base[0].re[3]);
    printf("Thread#%d im: %f %f %f %f\n", tinfo->thread_num, base[0].im[0], base[0].im[1], base[0].im[2], base[0].im[3]);
