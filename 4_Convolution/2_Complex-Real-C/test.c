...
int test3(struct _result *result, const struct _input *input) {
	...
	for (int j=0; j<=input->clen-4; j+=4) {
	    ...
	    data = LOADSIMD(&input->data_real[i+j]);   res_re0 += data * coeff;
	    data = LOADSIMD(&input->data_real[i+j+1]); res_re1 += data * coeff;
	    data = LOADSIMD(&input->data_real[i+j+2]); res_re2 += data * coeff;
	    data = LOADSIMD(&input->data_real[i+j+3]); res_re3 += data * coeff;

	    data = LOADSIMD(&input->data_imag[i+j]);   res_im0 += data * coeff;
	    data = LOADSIMD(&input->data_imag[i+j+1]); res_im1 += data * coeff;
	    data = LOADSIMD(&input->data_imag[i+j+2]); res_im2 += data * coeff;
	    data = LOADSIMD(&input->data_imag[i+j+3]); res_im3 += data * coeff;
	}
	result->data_real[i]   = res_re0[0] + res_re0[1] + res_re1[2] + res_re0[3];
...
int test4(struct _result *result, const struct _input *input) {
	...
	for (int j=0; j<=input->clen-4; j+=4) {
	    ...
	    data = LOADSIMD(&input->data_real[i+j+4]); res_re4 += data * coeff;
	    data = LOADSIMD_UNALIGNED(&input->data_real[i+j+5]); res_re5 += data * coeff;
	    data = LOADSIMD_UNALIGNED(&input->data_real[i+j+6]); res_re6 += data * coeff;
	    data = LOADSIMD_UNALIGNED(&input->data_real[i+j+7]); res_re7 += data * coeff;
	    ...
	    data = LOADSIMD(&input->data_imag[i+j+4]); res_im4 += data * coeff;
	    data = LOADSIMD_UNALIGNED(&input->data_imag[i+j+5]); res_im5 += data * coeff;
	    data = LOADSIMD_UNALIGNED(&input->data_imag[i+j+6]); res_im6 += data * coeff;
	    data = LOADSIMD_UNALIGNED(&input->data_imag[i+j+7]); res_im7 += data * coeff;
	}
...
int test5(struct _result *result, const struct _input *input) {
	...
	for (int j=0; j<=input->clen-4; j+=4) {
	    ...
	    data = LOADSIMD(&input->data_real[i+j+4]); res_re4 += data * coeff;
	    data = LOADSIMD(&input->data_real[i+j+5]); res_re5 += data * coeff;
	    data = LOADSIMD(&input->data_real[i+j+6]); res_re6 += data * coeff;
	    data = LOADSIMD(&input->data_real[i+j+7]); res_re7 += data * coeff;

	    data = LOADSIMD(&input->data_real[i+j+ 8]); res_re8  += data * coeff;
	    data = LOADSIMD(&input->data_real[i+j+ 9]); res_re9  += data * coeff;
	    data = LOADSIMD(&input->data_real[i+j+10]); res_re10 += data * coeff;
	    data = LOADSIMD(&input->data_real[i+j+11]); res_re11 += data * coeff;

	    data = LOADSIMD(&input->data_real[i+j+12]); res_re12 += data * coeff;
	    data = LOADSIMD_UNALIGNED(&input->data_real[i+j+13]); res_re13 += data * coeff;
	    data = LOADSIMD_UNALIGNED(&input->data_real[i+j+14]); res_re14 += data * coeff;
	    data = LOADSIMD_UNALIGNED(&input->data_real[i+j+15]); res_re15 += data * coeff;
	    ...
	    data = LOADSIMD(&input->data_imag[i+j+4]); res_im4 += data * coeff;
	    data = LOADSIMD(&input->data_imag[i+j+5]); res_im5 += data * coeff;
	    data = LOADSIMD(&input->data_imag[i+j+6]); res_im6 += data * coeff;
	    data = LOADSIMD(&input->data_imag[i+j+7]); res_im7 += data * coeff;

	    data = LOADSIMD(&input->data_imag[i+j+ 8]); res_im8  += data * coeff;
	    data = LOADSIMD(&input->data_imag[i+j+ 9]); res_im9  += data * coeff;
	    data = LOADSIMD(&input->data_imag[i+j+10]); res_im10 += data * coeff;
	    data = LOADSIMD(&input->data_imag[i+j+11]); res_im11 += data * coeff;

	    data = LOADSIMD(&input->data_imag[i+j+12]); res_im12 += data * coeff;
	    data = LOADSIMD_UNALIGNED(&input->data_imag[i+j+13]); res_im13 += data * coeff;
	    data = LOADSIMD_UNALIGNED(&input->data_imag[i+j+14]); res_im14 += data * coeff;
	    data = LOADSIMD_UNALIGNED(&input->data_imag[i+j+15]); res_im15 += data * coeff;
	}
...
int test6(struct _result *result, const struct _input *input) {
	...
	for (int j=0; j<=input->clen-8; j+=8) {
	    ...
	    data = LOADAVX(&input->data_real[i+j]);   res_re0 += data * coeff;
	    data = LOADAVX(&input->data_real[i+j+1]); res_re1 += data * coeff;
	    data = LOADAVX(&input->data_real[i+j+2]); res_re2 += data * coeff;
	    data = LOADAVX(&input->data_real[i+j+3]); res_re3 += data * coeff;

	    data = LOADAVX(&input->data_real[i+j+4]); res_re4 += data * coeff;
	    data = LOADAVX(&input->data_real[i+j+5]); res_re5 += data * coeff;
	    data = LOADAVX(&input->data_real[i+j+6]); res_re6 += data * coeff;
	    data = LOADAVX(&input->data_real[i+j+7]); res_re7 += data * coeff;


	    data = LOADAVX(&input->data_imag[i+j]);   res_im0 += data * coeff;
	    data = LOADAVX(&input->data_imag[i+j+1]); res_im1 += data * coeff;
	    data = LOADAVX(&input->data_imag[i+j+2]); res_im2 += data * coeff;
	    data = LOADAVX(&input->data_imag[i+j+3]); res_im3 += data * coeff;

	    data = LOADAVX(&input->data_imag[i+j+4]); res_im4 += data * coeff;
	    data = LOADAVX(&input->data_imag[i+j+5]); res_im5 += data * coeff;
	    data = LOADAVX(&input->data_imag[i+j+6]); res_im6 += data * coeff;
	    data = LOADAVX(&input->data_imag[i+j+7]); res_im7 += data * coeff;
	}
...
int test7(struct _result *result, const struct _input *input) {
	...
	for (int j=0; j<=input->clen-8; j+=8) {
	    ...
	    data = LOADAVX_UNALIGNED(&input->data_real[i+j+ 8]); res_re8  += data * coeff;
	    data = LOADAVX_UNALIGNED(&input->data_real[i+j+ 9]); res_re9  += data * coeff;
	    data = LOADAVX_UNALIGNED(&input->data_real[i+j+10]); res_re10 += data * coeff;
	    data = LOADAVX_UNALIGNED(&input->data_real[i+j+11]); res_re11 += data * coeff;

	    data = LOADAVX(&input->data_real[i+j+12]); res_re12 += data * coeff;
	    data = LOADAVX_UNALIGNED(&input->data_real[i+j+13]); res_re13 += data * coeff;
	    data = LOADAVX_UNALIGNED(&input->data_real[i+j+14]); res_re14 += data * coeff;
	    data = LOADAVX_UNALIGNED(&input->data_real[i+j+15]); res_re15 += data * coeff;
	    ...
	    data = LOADAVX_UNALIGNED(&input->data_imag[i+j+ 8]); res_im8  += data * coeff;
	    data = LOADAVX_UNALIGNED(&input->data_imag[i+j+ 9]); res_im9  += data * coeff;
	    data = LOADAVX_UNALIGNED(&input->data_imag[i+j+10]); res_im10 += data * coeff;
	    data = LOADAVX_UNALIGNED(&input->data_imag[i+j+11]); res_im11 += data * coeff;

	    data = LOADAVX(&input->data_imag[i+j+12]); res_im12 += data * coeff;
	    data = LOADAVX_UNALIGNED(&input->data_imag[i+j+13]); res_im13 += data * coeff;
	    data = LOADAVX_UNALIGNED(&input->data_imag[i+j+14]); res_im14 += data * coeff;
	    data = LOADAVX_UNALIGNED(&input->data_imag[i+j+15]); res_im15 += data * coeff;
	}
