...
int test2(struct _result *result, const struct _input *input) {
	...
	for (int j=0; j <= input->clen-4; j+=4) {
	    VECTYPE coeff;
            memcpy(&coeff, &input->coeff[j], sizeof(VECTYPE));
	    VECTYPE data;
            memcpy(&data, &input->data[i+j], sizeof(VECTYPE));
	    res += data * coeff;
	}
...
int test3(struct _result *result, const struct _input *input) {
	...
	for (int j=0; j<=input->clen-4; j+=4) {
	    VECTYPE data;
	    VECTYPE coeff;
	    memcpy(&coeff, &input->coeff[j], sizeof(VECTYPE)); // 1 x load coeff

	    memcpy(&data, &input->data[i+j], sizeof(VECTYPE));   res0 += data * coeff;
	    memcpy(&data, &input->data[i+j+1], sizeof(VECTYPE)); res1 += data * coeff;
	    memcpy(&data, &input->data[i+j+2], sizeof(VECTYPE)); res2 += data * coeff;
	    memcpy(&data, &input->data[i+j+3], sizeof(VECTYPE)); res3 += data * coeff;
	}
...
int test4(struct _result *result, const struct _input *input) {
	...
	for (int j=0; j<=input->clen-4; j+=4) {
	    ...
	    memcpy(&data, &input->data[i+j+4], sizeof(VECTYPE)); res4 += data * coeff;
	    memcpy(&data, &input->data[i+j+5], sizeof(VECTYPE)); res5 += data * coeff;
	    memcpy(&data, &input->data[i+j+6], sizeof(VECTYPE)); res6 += data * coeff;
	    memcpy(&data, &input->data[i+j+7], sizeof(VECTYPE)); res7 += data * coeff;
	}
...
int test5(struct _result *result, const struct _input *input) {
	...
	for (int j=0; j<=input->clen-4; j+=4) {
	    ...
	    memcpy(&data, &input->data[i+j+ 8], sizeof(VECTYPE)); res8  += data * coeff;
	    memcpy(&data, &input->data[i+j+ 9], sizeof(VECTYPE)); res9  += data * coeff;
	    memcpy(&data, &input->data[i+j+10], sizeof(VECTYPE)); res10 += data * coeff;
	    memcpy(&data, &input->data[i+j+11], sizeof(VECTYPE)); res11 += data * coeff;

	    memcpy(&data, &input->data[i+j+12], sizeof(VECTYPE)); res12 += data * coeff;
	    memcpy(&data, &input->data[i+j+13], sizeof(VECTYPE)); res13 += data * coeff;
	    memcpy(&data, &input->data[i+j+14], sizeof(VECTYPE)); res14 += data * coeff;
	    memcpy(&data, &input->data[i+j+15], sizeof(VECTYPE)); res15 += data * coeff;
	}
...
int test6(struct _result *result, const struct _input *input) {
	...
	for (int j=0; j<=input->clen-8; j+=8) {
	    AVXTYPE data;
	    AVXTYPE coeff;
            memcpy(&coeff, &input->coeff[j], sizeof(AVXTYPE)); // 1 x load coeff

	    memcpy(&data, &input->data[i+j], sizeof(AVXTYPE));   res0 += data * coeff;
	    memcpy(&data, &input->data[i+j+1], sizeof(AVXTYPE)); res1 += data * coeff;
	    memcpy(&data, &input->data[i+j+2], sizeof(AVXTYPE)); res2 += data * coeff;
	    memcpy(&data, &input->data[i+j+3], sizeof(AVXTYPE)); res3 += data * coeff;

	    memcpy(&data, &input->data[i+j+4], sizeof(AVXTYPE)); res4 += data * coeff;
	    memcpy(&data, &input->data[i+j+5], sizeof(AVXTYPE)); res5 += data * coeff;
	    memcpy(&data, &input->data[i+j+6], sizeof(AVXTYPE)); res6 += data * coeff;
	    memcpy(&data, &input->data[i+j+7], sizeof(AVXTYPE)); res7 += data * coeff;

	    memcpy(&data, &input->data[i+j+ 8], sizeof(AVXTYPE)); res8  += data * coeff;
	    memcpy(&data, &input->data[i+j+ 9], sizeof(AVXTYPE)); res9  += data * coeff;
	    memcpy(&data, &input->data[i+j+10], sizeof(AVXTYPE)); res10 += data * coeff;
	    memcpy(&data, &input->data[i+j+11], sizeof(AVXTYPE)); res11 += data * coeff;

	    memcpy(&data, &input->data[i+j+12], sizeof(AVXTYPE)); res12 += data * coeff;
	    memcpy(&data, &input->data[i+j+13], sizeof(AVXTYPE)); res13 += data * coeff;
	    memcpy(&data, &input->data[i+j+14], sizeof(AVXTYPE)); res14 += data * coeff;
	    memcpy(&data, &input->data[i+j+15], sizeof(AVXTYPE)); res15 += data * coeff;
	}
